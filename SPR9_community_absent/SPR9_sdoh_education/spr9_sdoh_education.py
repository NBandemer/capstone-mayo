# -*- coding: utf-8 -*-
"""SPR9_sdoh_education.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1obyFJFq131hUPkO96xTM6Cv5E8K_Qw9d
"""
...
best_checkpoint_path = os.path.join("/content/logs/epoch_logs/20240329_150748/checkpoint-704")
...
'''
softmax function exponentiates each element of the input vector and normalizes them
to make sure they sum to 1
'''

'''
binarizing labels is common in multi-class classification problems
because AUROC typically calculated for binary classification
binarizing labels allows you to treat each class as the positive class once,
while all other classes are considered as the negative class.
This allows evaluation of the model's ability to distinguish between classes.
'''
...
if num_labels == 2:
    ...
    # applies the softmax function along last dimension of the tensor, converts raw scores into probabilities
    probabilities = torch.softmax(torch.from_numpy(logits), dim=-1).numpy()

    # this selects the probabilities corresponding to the second class for each sample
    prob = probabilities[:, 1]

    # compare true labels with predicted probabilities for the positive class
    auroc = roc_auc_score(true_labels, prob)
elif num_labels > 2:
    # Multi-class classification

    # convert logits into probabilities for probability distribution over the classes for each sample
    probabilities = torch.softmax(torch.from_numpy(logits), dim=-1).numpy()

    # Binarize the labels in a one-vs-all fashion
    # each class will be treated as the positive class once, while all other classes are considered negative class
...
# determine if binary or multiclass classification based on the shape of the probabilities array
'''
ex: if probabilities is a 2D array, probabilities.shape will return (n_samples, n_classes)
probabilities.shape[1] accesses second element of the shape tuple, which corresponds to the number of classes
'''

num_classes = probabilities.shape[1] # this will return n_classes
...
if num_classes == 2:
    ...
    plt.figure()
    plt.plot(fpr, tpr, color='darkorange', lw=2, label='ROC curve (area = %0.2f)' % auroc) # plot the ROC curve
    ...
    plt.xlabel('False Positive Rate') # set label for x-axis
    plt.ylabel('True Positive Rate') # set label for y-axis
    plt.title('ROC')  # set title for the plot
    plt.legend(loc="lower right")  # set legend to plot
    plt.savefig(f"{current_sdoh}_roc_curve.png")
    plt.show()
    ...
    fpr = dict() # Initialize a dictionary to store false positive rates for each class
    tpr = dict() # Initialize a dictionary to store true positive rates for each class
    roc_auc = dict() # Initialize a dictionary to store AUC scores for each class
...
if num_classes == 2:
    ...
    plt.legend(loc="lower right")
    plt.savefig(f"{current_sdoh}_roc_curve_optimal_threshold.png")
    plt.show()
else:
    # Multi-class classification
    # Compute ROC curve and ROC area for each class
    ...
    for i in range(num_classes):
      ...
      print(f'Class {i}: Best Threshold = {best_thresholds[i]:.2f}')
...
plt.savefig(f"{current_sdoh}_cm.png")
plt.show()
